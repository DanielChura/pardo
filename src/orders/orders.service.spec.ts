...
describe('OrdersService', () => {
  ...
  describe('create', () => {
    ...
    const items = [{ productVariantId, quantity: 2 }];
    const variant = {
      id: productVariantId,
      stock: 10,
      price: 5000,
    ...
    it('should throw on when variant does not exist', async () => {
      ...
      mockPrisma.order.findUnique.mockReturnValue(null);
      expect(orderService.findOne(userId, orderId)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw on decrement stock', async () => {
      ...
      const items = [{ productVariantId, quantity: 200 }];
      mockPrisma.productVariant.findUnique.mockReturnValue(variant);
      await expect(orderService.create(userId, { items })).rejects.toThrow(
        BadRequestException,
      );
    });
  });
